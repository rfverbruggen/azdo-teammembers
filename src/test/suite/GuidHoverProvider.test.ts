...
suite('GuidHoverProvider Test Suite', () =>
{
	test('ValidConfig_HoverGuid_Success', async () =>
	{
		...
		// Act.
		let hover = <vscode.Hover[]>(await vscode.commands.executeCommand("vscode.executeHoverProvider", (await document).uri, new vscode.Position(2, 10)));

		// Assert.
        assert.strictEqual((<{ language: string; value: string }>hover[0].contents[0]).value, expectedAnswer);
	});
});
